<template>
	<view>
		...
		<!-- 图片部分 -->
		<view class="image">
			<image v-for="(item,index) in imgurl" :src="getImgsrc(item)" mode="" ></image>
		</view>
		<view class="images" @click="getimg()">
			<view class="jia">+</view>
...
<script>
	export default {
		...
		methods: {
			...
			//发布知识
			async gofabu(){
				if(this.fenleiid != null && this.title != '' && this.miaoshu != '' && this.imgurl.length != 0 && this.thenff != null){
