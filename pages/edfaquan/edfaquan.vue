...
<script>
	export default {
		...
		methods: {
			//获取当前发圈的详情
			async getfaquaninfor(){
				const req = await uni.$http.get('api/jiu/scrunchie/edit',{
					id:this.allid
				})
				console.log(req)
				this.allfqlist = req.data.data.action
				// console.log(this.allfqlist.content)
				this.title = this.allfqlist.title
				this.usertext = this.allfqlist.content
				this.imgurl = this.allfqlist.image
				this.fuwu = this.allfqlist.lei.name
				this.topictext = this.allfqlist.topic.name
				this.topic = this.allfqlist.topic.id
				this.cate_id = this.allfqlist.lei.id
			},
			
			//发布知识
			async gofabu(){
				if(this.fenleiid != null && this.title != '' && this.usertext != ''){
					const req = await uni.$http.get('api/jiu/scrunchie/quanedit',{
