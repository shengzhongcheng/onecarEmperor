<template>
	...
	<view class="receiving-2">
		<view class="list">
			<view>收货人</view>
			<view>
				<input type="text" style="text-align: right;" placeholder='请填写收货人姓名' v-model='addressList.real_name'>
			</view>
		</view>
		<view class="list">
			<view>手机号码</view>
			<view>
				<input type="text" style="text-align: right;" placeholder='请填写收货人手机号' v-model='addressList.phone'>
			</view>
		</view>
		<view class="list">
			<view>选择所在地区</view>
		...
		<view class="list">
			<view>详细地址</view>
			<view class="area">
				<input type="text" placeholder="输入详情地址" v-model="addressList.detail">
			</view>
		</view>
		<view class="list">
			<view>设置默认地址</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 		detail: this.detail
			// 	}
			// 	that.$http.postAddressMap(p)
			// 		.then(res => {
			// 			console.log('save', res)
			// 			that.saveList = res.data.data
			// 			console.log('save', that.saveList)
			// 		})
			// },
			// 请求修改地址数据
			getEditAddres() {
				...
				that.$http.getEditAddress(d)
					.then(res => {
						console.log('1111', res)
						that.addressList = res.data.data
						console.log('222', that.addressList)
					})
			},
			//确认选择默认
			checkChange(e) {
			...
			submitBox() {
				let _this = this
				let data = {
					real_name: _this.addressList.real_name,
					phone: _this.addressList.phone,
					city: _this.multiArray[0][_this.multiIndex[0]] + '-' + _this.multiArray[1][_this.multiIndex[1]] +
						'-' + _this.multiArray[2][_this.multiIndex[2]],
					detail: _this.addressList.detail,
					is_default: _this.addressList.is_default,
					id: _this.addressList.id
				}
				_this.$http.postAddressMap(data)
					.then(res => {
