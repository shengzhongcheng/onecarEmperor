<template>
	<view>
		...
		<view class="usertitle" v-if="shopinfor.length!=0">
			<!-- 用户信息 -->
			<!-- <view class="user">
			...
			<!-- 内容标题 -->
			<view class="title" style="width: 100%;text-align: center;">{{shopinfor.question}}</view>
			<!-- 时间 -->
			<view class="time" style="width: 100%;text-align: center;margin-bottom: 20rpx;font-size: 20rpx;">{{shopinfor.user.nickname}} {{shopinfor.createtime | timeAgo}}</view>
			<!-- 内容标题 -->
			...
			<!-- 图片 -->
			<view class="image"><image @click="lookimg(index)" v-for="(item,index) in shopinfor.images" :key="index" :src="getImgsrc(item)" mode="aspectFit"></image> </view>
			<!-- 视频 -->
...
<script>
	export default {
		...
		methods: {
			...
			lookimg(index){
				var arr = []
				for(let i in this.shopinfor.images){
					
					let urlk = this.shopinfor.images[i];
					let str = RegExp('http');
					let newUrl;
			...
			async pinglungo(){
				...
				const req = await uni.$http.post('api/v1/answer/comment',{
					...
					flag:1,//身份:0=用户,1=店铺/师傅
					moment_type:1,//来源类型:1=知识学堂,2=车圈
				})
				console.log(req)
			...
			//获取详情
			async getinfor(){
				const req = await uni.$http.get('api/jiu/answer/andetail',{
					id:this.id,
